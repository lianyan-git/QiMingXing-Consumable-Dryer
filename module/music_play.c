 ...
 * 播放占用 TIM3（bz 时基）：按音符改 ARR 与 CH4 占空，背光(CH3)不动——
 * 背光 PWM 频率随音符变化导致屏幕有闪动属预期（用户接受），换得音色自然。
 * 进度推进由 MusicPlay_Poll 每主循环调用。
 */
...
static uint16_t s_cur_freq = 0;         /* 当前音符频率(0=休止), 供RGB音高灯效 */

/* 按音符改 ARR；freq=0 表示休止→关输出。背光(CH3)不参与。 */
...
uint16_t MusicPlay_CurFreq(void)            { return s_cur_freq; }
